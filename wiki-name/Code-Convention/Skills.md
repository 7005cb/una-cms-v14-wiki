# UNA CMS V14 – Coding Conventions

This document defines the coding standards for UNA CMS V14. All generated and modified code must follow these rules.

## Quick rules for AI coding agents

When generating or refactoring UNA CMS code, always:

- Use **clean, readable PHP/JS/HTML/CSS** with consistent indentation (4 spaces, no tabs).
- Follow UNA’s naming and structure rules (see sections below).
- Use **bx_import()** instead of `require_once`/`include` for classes.
- Validate all user input with **bx_process_input()** and output with **bx_process_output()**.
- Use **prepared statements** via `BxDolDb::prepare()` in dedicated query classes.
- Inherit core classes from **BxDol** (no `BxDolMistake`).
- Use singleton pattern via `Class::getInstance()` for core singletons.
- Keep HTML/CSS in templates, not inline in PHP logic.
- Add proper file headers/footers and docblocks.

If a request conflicts with these rules, prefer these conventions and explain the deviation.

---

**UNA** programming approaches will help everyone to write more secure and easy to read code when developing modules and modifications.
...
The following classes are already changed and call to this functions need to be changed, where needed. In user part template object can be detected by default, but in admin part Template object need to be passed explicitly:
...
Call **BxDolDb::prepare** function in particular DB class function for your functionality. If there is no such class - create it; every piece of code should have SQL queries in a separate class.
...
Don't call `bx_process_input` and/or `bx_process_output` in DB class function, call these functions in the code. It is better to call `bx_process_input` function as early as possible, before using any user/untrusted input.
...
There are some predefined documentation pages where you can add content. For example "Objects" documentation page consists of references to all "Objects".
