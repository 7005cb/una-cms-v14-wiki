# UNA CMS V14 – Directory Structure

This document describes the directory structure of UNA CMS V14 and how modules should use it.

## Quick rules for AI coding agents

When generating or modifying UNA CMS code:

- Assume the directory layout described below; do not invent new top-level folders.
- For modules, use the standard module structure (`/classes/`, `/install/`, `/template/`, etc.).
- Prefer system folders (`/logs/`, `/tmp/`, `/cache/`, `/cache_public/`, `/storage/`) over module-local folders for logs, temp files, cache, and user files.
- Access `/storage/` only via the **Storage** object class, not directly.
- Respect the differences from Dolphin 7 (no `/administration/`, use `/studio/`, templates and languages are modules, etc.).

If a requested file path or module layout conflicts with this, adjust to match this structure.

---

## UNA directories structure

`/cache/` - cache files which should be inaccessible from the web (DB queries cache, language files cache, etc.)  
...
`/plugins/` - 3rd-party libraries which should be inaccessible from the web (usually PHP libraries)  
...
4. No more `/administration/` folder; instead there is `/studio/`, which contains functionality related to site configuration only. Other administration functionality is moved to the user side controlled by membership levels; there are built-in **Moderator** and **Administrator** ACL levels.  
